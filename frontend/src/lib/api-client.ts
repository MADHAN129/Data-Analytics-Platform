...
class ApiClient {
  ...
  async listDashboards(params?: PaginationParams): Promise<DashboardListResponse> {
    const query = new URLSearchParams()
    if (params?.page) query.set("page", String(params.page))
    if (params?.per_page) query.set("per_page", String(params.per_page))
    return this.request<DashboardListResponse>(`/dashboards?${query.toString()}`)
  }

  async createDashboard(data: DashboardCreateRequest): Promise<DashboardDetailResponse> {
    return this.request<DashboardDetailResponse>("/dashboards", {
      method: "POST",
      body: JSON.stringify(data),
    })
  }

  async getDashboardById(id: number): Promise<DashboardDetailResponse> {
    return this.request<DashboardDetailResponse>(`/dashboards/${id}`)
  }

  async updateDashboard(id: number, data: DashboardUpdateRequest): Promise<DashboardDetailResponse> {
    return this.request<DashboardDetailResponse>(`/dashboards/${id}`, {
      method: "PUT",
      body: JSON.stringify(data),
    })
  ...
  async deleteDashboard(id: number): Promise<void> {
    await this.request<MessageResponse>(`/dashboards/${id}`, { method: "DELETE" })
  }

  async addWidget(dashboardId: number, data: WidgetConfig): Promise<WidgetResponse> {
    return this.request<WidgetResponse>(`/dashboards/${dashboardId}/widgets`, {
      method: "POST",
      body: JSON.stringify(data),
    })
  }

  async updateWidget(dashboardId: number, widgetId: number, data: WidgetConfig): Promise<WidgetResponse> {
    return this.request<WidgetResponse>(`/dashboards/${dashboardId}/widgets/${widgetId}`, {
      method: "PUT",
      body: JSON.stringify(data),
    })
  }
  ...
  async deleteWidget(dashboardId: number, widgetId: number): Promise<void> {
    await this.request<MessageResponse>(`/dashboards/${dashboardId}/widgets/${widgetId}`, { method: "DELETE" })
  }

  async updateDashboardLayout(dashboardId: number, data: LayoutUpdateRequest): Promise<WidgetResponse[]> {
    return this.request<WidgetResponse[]>(`/dashboards/${dashboardId}/layout`, {
      method: "PUT",
      body: JSON.stringify(data),
    })
  }

  async autoGenerateDashboard(data: AutoGenerateRequest): Promise<DashboardDetailResponse> {
    return this.request<DashboardDetailResponse>("/dashboards/auto-generate", {
      method: "POST",
      body: JSON.stringify(data),
    })
  }
